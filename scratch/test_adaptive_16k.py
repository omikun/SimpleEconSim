...
def generate_16k_mesh(gen: PolygonMapGenerator, width: int = 1000, height: int = 1000,
    ...
    # Base triangles
    triangles = [] # (pa, pb, pc, col, avg_elev, is_river, area)
    ...
    for edge in gen.edges:
        ...
        if not d0.water:
            ...
            add_tri(p_d0, p_v0, p_mid, col0, (d0.elevation + v0.elevation)*0.5, edge.river > 0)
            add_tri(p_d0, p_mid, p_v1, col0, (d0.elevation + v1.elevation)*0.5, edge.river > 0)

        if not d1.water:
            p_d1 = np.array([d1.x * scale_x, d1.y * scale_y, d1.elevation * elev_scale], dtype=np.float64)
            add_tri(p_d1, p_v1, p_mid, col1, (d1.elevation + v1.elevation)*0.5, edge.river > 0)
            add_tri(p_d1, p_mid, p_v0, col1, (d1.elevation + v0.elevation)*0.5, edge.river > 0)

    print(f"Initial base triangles: {len(triangles)}")

    # Adaptive subdivision loop to hit target_polys
    rng_seed = 42
    while len(triangles) < target_polys:
        needed = target_polys - len(triangles)
        num_to_subdiv = min(len(triangles), max(1, needed // 3))
        
        # Sort by area so largest triangles get subdivided first
    ...
    print(f"Target: {target_polys} | Result: {len(triangles)} micropolygons")
    return triangles

def render_mesh_to_surface(gen: PolygonMapGenerator, triangles, width: int = 1000, height: int = 1000):
...
def main():
    ...
    t0 = time.time()
    tris = generate_16k_mesh(gen, 1000, 1000, target_polys=16000, roughness=5.0)
    surf = render_mesh_to_surface(gen, tris, 1000, 1000)
    t1 = time.time()
    print(f"Total time for 16k mesh generation and rendering: {(t1 - t0)*1000:.2f} ms")

    buf = pygame.image.tostring(surf, "RGB")
    arr = np.frombuffer(buf, dtype=np.uint8).reshape((1000, 1000, 3))

    out_path = os.path.join(ARTIFACT_DIR, "exact_16k_polys.png")
    plt.figure(figsize=(10, 10), dpi=150)
    plt.imshow(arr)
    plt.title(f"Procedural Elevation: 16k Micropoly Island ({len(tris):,} Polygons)\n(https://www.redblobgames.com/x/1725-procedural-elevation/#rendering)",
