...
# Simulate droplets with stronger carving for crisp mountain gullies
sim = HydraulicErosionSim(grid_size=grid_size, seed=42)
land_mask = base_grid > 0.02
eroded_grid, _ = sim.simulate_droplets(base_grid, num_droplets=15000, land_mask=land_mask, carving_scale=0.65)
eroded_grid = sim.apply_thermal_erosion(eroded_grid, iterations=3)
...
for edge in gen.edges:
    ...
    # If river flows along v0-v1, depress the river bed slightly for a natural V-canyon
    if edge.river > 0:
        z_v0 -= min(3.0, edge.river * 0.8)
        z_v1 -= min(3.0, edge.river * 0.8)
...
def get_midpoint(i_a, i_b, depth):
    ...
    if length > 1.2:
        ...
        fbm_val = procedural_fbm_elevation(mid[0], mid[1], 42, amplitude=elev_scale / 70.0) * (decay * 0.35)
        mid_norm_elev = base_h / elev_scale
        rdg_val = procedural_ridged_elevation(mid[0], mid[1], mid_norm_elev, 42, amplitude=elev_scale / 45.0, ridge_roughness=0.45) * (decay * 0.5)
        mid[2] = 0.75 * base_h + 0.25 * mid[2] + fbm_val + rdg_val + rng.uniform(-0.2, 0.2) * (length / 30.0) * decay
...
pygame.image.save(surf, "scratch/test_3d_erosion_channels.png")
print("Saved preview to scratch/test_3d_erosion_channels.png")
